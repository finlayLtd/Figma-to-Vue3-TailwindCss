...
class Admin {
	...
	function ippool($page = 1, $reslen = 50, $post = array()){
		if(empty($post)){
			...
			$path = 'index.php?act=ippool&poolname='.urlencode($post['poolname']).'&poolgateway='.$post['poolgateway'].'&netmask='.$post['netmask'].'&nameserver='.$post['nameserver'].'&servers_search='.$post['servers_search'].'&page='.$page.'&reslen='.$reslen;
			$res = $this->call($path);
		}
		return $res;
	}
